...
function tokenBurn(event: ethereum.Event, from: Address, value: BigInt, index: BigInt): void {
  ...
  let poolReserve = getOrInitReserve(Address.fromBytes(bToken.underlyingAssetAddress), event);
  ...
  let userReserve = getOrInitUserReserve(from, Address.fromBytes(bToken.underlyingAssetAddress), event);
...
export function handleBTokenBurn(event: BTokenBurn): void {
  let bToken = getOrInitBToken(event.address);
  let poolReserve = getOrInitReserve(Address.fromBytes(bToken.underlyingAssetAddress), event);
  let userReserve = getOrInitUserReserve(event.params.from, Address.fromBytes(bToken.underlyingAssetAddress), event);
...
export function handleDebtTokenBurn(event: DebtTokenBurn): void {
  ...
  let userReserve = getOrInitUserReserve(from, Address.fromBytes(dToken.underlyingAssetAddress), event);
  let poolReserve = getOrInitReserve(Address.fromBytes(dToken.underlyingAssetAddress), event);
