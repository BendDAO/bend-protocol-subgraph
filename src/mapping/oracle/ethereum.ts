...
export function priceFeedUpdated(
  ...
  let proxyPriceProvider = AaveOracle.bind(Address.fromString(priceOracle.proxyPriceProvider.toHexString()));
...
export function handleChainlinkAggregatorUpdated(event: AggregatorUpdated): void {
  ...
  if (priceOracle.version == 1) {
    chainLinkAggregatorUpdated(event, assetAddress, assetOracleAddress, priceOracleAsset, priceOracle);
  } else {
    log.error(`Event should not have been called for version > 1 || asset: {} | oracleAddress: {}`, [
      assetAddress.toHexString(),
      assetOracleAddress.toHexString(),
    ]);
  }
